...
use crate::{
    Apply, Element, theme,
    widget::{button, container, row, space::vertical},
};
...
impl<'a, Message: Clone + 'static> ListColumn<'a, Message> {
    ...
    pub fn into_element(self) -> Element<'a, Message> {
        ...
        let list_item_padding = self.list_item_padding;
        let radius_s = theme::active().cosmic().radius_s();

        let mut content = Vec::with_capacity(count);

        for (i, item) in self.children.into_iter().enumerate() {
            let is_first = i == 0;
            ...
            match item {
                ListItem::Element(element) => {
                    ...
                    let list_item = row![
                        container(element).align_y(iced::Alignment::Center),
                    ...
                    .class(theme::Container::custom(move |theme| {
                        let mut style = theme.style(&theme::Container::List);
                        style.border.radius = radius;
                        style
                    }));

                    content.push(list_item.into());
                }
...
fn get_radius(radius: [f32; 4], first: bool, last: bool) -> [f32; 4] {
    match (first, last) {
        (true, true) => [radius[0], radius[1], radius[2], radius[3]],
